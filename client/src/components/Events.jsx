    ...
    return (
        <>
            ...
            <div className="py-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 px-[5%]  gap-4 py-6 ">
                    {
                        event_data.map((event,i) => (
                            <div key={i} className="shadow-md hover:shadow-lg transition-shadow duration-350  min-w-[250px] h-full  gap-8 m-auto  bg-white/5 border border-zinc-300/40 rounded-3xl  overflow-clip"> 
                              {/* <Image className="z-0 object-cover aspect-[4/3] w-full" src={event.image}  radius="none" alt="" /> */}
                                <div className="flex flex-col w-full m-auto p-[5%]">
                                    <h1 className="font-bold text-xl uppercase">{event.tittle.length === 0 ? "Coming Soon" :event.tittle} </h1>
